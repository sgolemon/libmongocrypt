...
_test_roundtrip_single (const _test_mc_crypto_roundtrip_t *test)
{
   ...
   _mongocrypt_buffer_init_size (
      &out, test->algo->ciphertext_len (test->plaintext.len, status));
   ASSERT_OK_STATUS (true, status);
   uint32_t outlen;
   bool ret = test->algo->encrypt (crypt->crypto,
                                   &test->iv,
                                   &test->aad,
                                   &test->key,
                                   &test->plaintext,
   ...
   if (test->encrypt_error) {
      ...
      goto done;
   } else if (test->ignore_encrypt_failure) {
   ...
   const uint32_t plaintext_len =
      test->algo->plaintext_len (test->ciphertext.len, status);
   ...
   ret = test->algo->decrypt (crypt->crypto,
                              &test->aad,
                              &test->key,
                              &test->ciphertext,
                              &out,
   ...
   out.len = test->algo->plaintext_len (test->ciphertext.len, status);
...
get_algo_by_name (const char *name)
{
   ...
   if (!strcmp (name, "AES-256-CTR/NONE") || !strcmp (name, "FLE2IEV")) {
      return _mcFLE2IEVAlgorithm ();
   }
...
_parse_roundtrip_test (bson_iter_t *iter, _test_mc_crypto_roundtrip_t *test)
{
   while (bson_iter_next (iter)) {
      ...
      } else if (!strcmp (field, "ignore_encrypt_failure")) {
         ASSERT_OR_PRINT_MSG (
            !test->ignore_encrypt_failure,
            "Duplicate field 'ignore_encrypt_failure' in test");
         ASSERT (BSON_ITER_HOLDS_BOOL (iter));
         ASSERT_OR_PRINT_MSG (bson_iter_bool (iter),
                              "value of 'ignore_encrypt_failure' must be true");
         test->ignore_encrypt_failure = true;
      }
...
#undef HEXBUF_FIELD
   ...
   ASSERT_OR_PRINT_MSG (test->plaintext.data || test->encrypt_error,
                        "Missing field 'plaintext'");
...
{
   printf ("Loading tests from %s...", path);

   mongocrypt_binary_t *test_bin = TEST_FILE (path);
   if (!test_bin) {
      TEST_ERROR ("Failed loading test data file '%s'", path);
   }
   ...
   printf ("Finished tests in %s", path);
}
